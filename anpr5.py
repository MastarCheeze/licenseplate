...
class PyImageSearchANPR:
	...
	def locate_license_plate(self, gray, candidates, thresh, clearBorder):
		...
		for i, c in enumerate(candidates):
			...
			cv2.drawContours(mask, [box], -1, 255, -1) # Draw filled contour in mask
			...
			# 	licensePlate = imutils.resize(licensePlate, height=500)
			...
			# 	roi = imutils.resize(licensePlate, height=55)
...
for filename in ["zh1_right.jpg"]:# os.listdir("./carplates_newnew"):
	...
	if (lpText is not None) and (len(lpText) == 8):
		...
		print("hi")
		image = imutils.resize(image, width=image.shape[1]*2, height=image.shape[0])
		lpText, lpCont = anpr_strech.find_and_ocr(image, clearBorder=True)
